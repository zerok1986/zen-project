...
const Portal = () => {
  ...
  const findActivity = (activity) => {
    let copy = activitiesInitial.filter(
      (elm) =>
        elm.name.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, "").includes(activity.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, "")) ||
        elm.type.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, "").includes(activity.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, ""))
    )
    setList(copy)
  }
