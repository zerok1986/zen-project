...
router.get("/allActivities", (req, res) => {
  Activity.find()
    ...
    .catch(err => res.json({ err, errMessage: "Problema buscando activities" }))
})
...
router.get("/activity/:id", (req, res) => {
  ...
  Activity.findById(id)
    ...
    .catch(err => res.json({ err, errMessage: "Problema buscando una Activity" }))
})
...
router.post("/newActivity", (req, res) => {
  ...
  console.log("object")
  Activity.create({ name, type, maxAssistants, date, location, price, duration, teacher })
    .then(newActivity => {console.log("newActivity"),res.json(newActivity)})
